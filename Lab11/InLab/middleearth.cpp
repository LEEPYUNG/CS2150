 ...
 *
 *Shuffles the contents of the vector specified 
 *
 *@param first A string iterator to iterate through the vector 
 *@param last  A string iterator to iterate through the vector
 *@param g     A memory address to a marsenne twister random number engine
 *
 *
 */
 ...
 *
 *Construction of the this class specifying the x and y axis sizes, number of cities and the seed.
 *
 *@param xsize The size of xaxis
 *@param ysize The size of yaxis
 ...
 *
 *Prints out all the cities in the created world.                         
 *
 */
...
/**
 *
 *@brief Prints a tab-separated table of the distances to command window
 *
 *Creates a table that shows the distances between cities that can be put into Excel/ect
 *                         
 *
...
void MiddleEarth::printTable() {
    cout << "Table: " << endl << endl << "~/Desktop/Table";
    for (auto city : cities) {
        cout << city << "\t";
    }
    cout << endl;
...
/**
 *
 *@brief Gets the distance between 2 cities.
 *
 *Given two cities as input, the getDistance will use the distance 2D array to get the float distance between them..
 *
 *@return returns a float number representing the distance between cities.
...
/**
 *
 *@brief given a e length, returns the cities that must be visited.
 *
 *This function takes in an unsigned int length (the number of cities) and will return
 *a vector of strings containing all the cities that must be visited. 
 *This function does not include the first and last cities (there will be length +1 entries)
 *
 *
 *@return returns a vector of strings containing the cities,
